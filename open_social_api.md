...
**Important :** *userId* must contain the contextId (in this case, the id of the space), *personId* must contain the contextType (in this case, **@space**). *groupId* will be ignored.
...
Get the name of everyone that has join the space with id 3253 :
...
Retrieve the name of user with id 3253 :
...
*contextId* as well as the *contextType* must be specified.

#### List subspaces of a space

Retrieve all informations about subspaces of the space that as id 3253 :
...
Retrieve all informations about top level spaces (only the ones that appears on its main page, subspaces of them are not retrieved) of user with id 1318 :
...
Retrieve the name of space with id 3437 :
...
### RETRIEVE THE ACTIVITY STREAM OF A SPACE

### RETRIEVE THE ACTIVITY STREAM OF A USER
